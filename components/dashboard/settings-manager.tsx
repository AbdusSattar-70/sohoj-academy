...
export function SettingsManager({years,classes,programs,subjects,batches}:{years:(Opt&{starts_on:string;ends_on:string;is_active:boolean})[];classes:(Opt&{sort_order:number})[];programs:(Opt&{code:string|null})[];subjects:(Opt&{code:string|null})[];batches:(Opt&{capacity:number})[]}) {
  return <div className="grid gap-6 xl:grid-cols-2">
    <Panel title="Academic Years" rows={years.map(x=>[x.name,`${x.starts_on} → ${x.ends_on}${x.is_active?" • Active":""}`])}>
      <ActionForm action={createAcademicYear} successMessage="Academic year added." className="grid gap-2 sm:grid-cols-2"><Input name="name" placeholder="2026" required/><Input name="starts_on" type="date" required/><Input name="ends_on" type="date" required/><label className="flex items-center gap-2 text-sm"><input name="is_active" type="checkbox"/> Active year</label><Button className="sm:col-span-2">Add Academic Year</Button></ActionForm>
    </Panel>
    <Panel title="Classes" rows={classes.map(x=>[x.name,`Order ${x.sort_order}`])}>
      <ActionForm action={createClass} successMessage="Class added." className="flex gap-2"><Input name="name" placeholder="Class 8" required/><Input name="sort_order" type="number" placeholder="8"/><Button>Add</Button></ActionForm>
    </Panel>
    <Panel title="Programs" rows={programs.map(x=>[x.name,x.code??"—"])}>
      <ActionForm action={createProgram} successMessage="Program added." className="flex gap-2"><Input name="name" placeholder="SSC A+ Preparation" required/><Input name="code" placeholder="SSC-A+"/><Button>Add</Button></ActionForm>
    </Panel>
    <Panel title="Subjects" rows={subjects.map(x=>[x.name,x.code??"—"])}>
      <ActionForm action={createSubject} successMessage="Subject added." className="flex gap-2"><Input name="name" placeholder="Mathematics" required/><Input name="code" placeholder="MATH"/><Button>Add</Button></ActionForm>
    </Panel>
    <Panel title="Batches" rows={batches.map(x=>[x.name,`Capacity ${x.capacity}`])}>
      <ActionForm action={createBatch} successMessage="Batch added." className="grid gap-2 sm:grid-cols-2">
